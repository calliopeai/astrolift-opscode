...
#
# Tiered up from stg: General Purpose D4s_v3 SKU, zone-redundant HA,
# 35-day backup retention, geo-redundant backup gated by toggle (default
# on in prd), and one read replica in zone 3 for read scale-out + DR.
# -----------------------------------------------------------------------------
...
# Read replica for read scale-out + warm-standby DR. Lives in zone 3 so
# both the primary and its standby (zones 1+2) can fail without losing
# read traffic.
