# -----------------------------------------------------------------------------
# Virtual Network + Subnets + NAT Gateway (production)
#
# Subnets are sized off local.vnet_cidr (10.100.0.0/16). NAT topology is
...
resource "azurerm_subnet" "cache" {
  ...
  resource_group_name  = azurerm_resource_group.main.name
  virtual_network_name = azurerm_virtual_network.main.name
  address_prefixes     = ["10.100.21.0/24"]
}
...
resource "azurerm_subnet" "container_apps" {
  ...
  resource_group_name  = azurerm_resource_group.main.name
  virtual_network_name = azurerm_virtual_network.main.name
  address_prefixes     = ["10.100.32.0/23"]
...
# -----------------------------------------------------------------------------
# NAT Gateway (per-zone for prd — survives single-zone outage)
# -----------------------------------------------------------------------------
