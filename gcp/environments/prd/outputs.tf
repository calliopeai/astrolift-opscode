...
output "cloudsql_private_ip" {
  description = "Cloud SQL primary private IP address (reachable via VPC peering)"
  value       = google_sql_database_instance.postgres.private_ip_address
}

output "cloudsql_replica_private_ips" {
  description = "Cloud SQL read replica private IPs (one per replica; empty list when db_read_replica_count = 0)"
