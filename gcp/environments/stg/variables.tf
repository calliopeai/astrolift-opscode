...
variable "base_domain" {
  description = "Base DNS zone for this environment (e.g. stg.acme.com). Operator must own this zone or have NS delegation to Cloud DNS."
  type        = string
}
...
variable "db_read_replica_count" {
  description = "Number of Cloud SQL read replicas (0 disables; replicas inherit tier from primary unless overridden)"
  type        = number
  default     = 0
}
